...
The `ConfigFile()` class reads a given text file and creates a table of colon deliminated `key: value` pairs. This allows various gameplay paramters such as score values awarded and deducted from the player, procedural colour values, and random percaentages to be altered without recompiling the application from source. This allows non-technical members of the team to test out ideas autonomously without the need to wait for a programmer.
...
The config file is also able to handle incosistent whitespace and comments starting with a `#` symbol.
...
The other getter is the getTextUnit function which returns the GLunit containing the information about the texture unit of the pressed sprite.  
